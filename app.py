...
def apply_section_filter(_df: pd.DataFrame, selected_sections: list) -> pd.DataFrame:
    ...
    # Optional: matched-section sort key (keeps "Section > Student" aligned after filtering)
    def _matched_section_key(v):
        vals = _sections_iter(v)
        nums = [int(x) for x in vals if x.isdigit() and x in sel]
...
if subject_sections_up is not None:
    ...
    # pick a likely column (or fall back to first)
    sec_col_guess = None
...
if sections_up is not None:
    ...
    if sid_sections and section_col:
        ...
        # Room -> Teacher mapping (CK Price). Used only for labeling the dropdown.
        TEACHER_BY_ROOM = {
            "23": "Allen",
            "31": "Bennett",
            "18": "Cabral",
        ...
        room_labels = [
            f"{r} - {TEACHER_BY_ROOM.get(str(r), '')}".rstrip(" -")
        ...
        # Pull just the room number back out ("20 - Coorough" -> "20")
        selected_rooms = [lbl.split(" - ")[0].strip() for lbl in selected_room_labels]
...
# If sorting by section, ensure we have a numeric _sort_section key.
# - When filtering by room/section, apply_section_filter already adds _sort_section based on matched sections.
# - For whole-school runs (no section filter), optionally use the uploaded subject section list to pick the
#   student’s Math/ELA section out of their many sections.
if sort_choice == "Section > Student" and "SectionsList" in view.columns and "_sort_section" not in view.columns:
    if subject_sort_sections:
        ...
        # Fallback: if selected_sections is set but apply_section_filter didn't add _sort_section for some reason
        sel = set(str(s).strip() for s in selected_sections)
